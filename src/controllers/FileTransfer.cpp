    ...
    while (!m_receiveBuffer.isEmpty()) {
        if (m_receiveState == WaitingForHeader) {
            ...
            if (!m_saveDirectory.isEmpty()) {
                // 저장 디렉토리가 설정된 경우
                QDir saveDir(m_saveDirectory);
                if (!saveDir.exists()) {
                    saveDir.mkpath(".");
                }
                fullPath = saveDir.absoluteFilePath(fileName);
            } else {
                // 기본: 현재 디렉토리
                fullPath = fileName;
            }

            m_receiveFile = new QFile(fullPath);
            ...
            qDebug() << "File receive started:" << fullPath << "Size:" << m_totalReceiveBytes;
            emit fileReceiveStarted(fullPath, m_totalReceiveBytes);  // 전체 경로 전달
        }
            ...
            if (m_receivedBytes >= m_totalReceiveBytes) {
                // 수신 완료 - 전체 경로 전달
                QString completedPath = m_receiveFile->fileName();
                qDebug() << "File receive completed:" << completedPath;
                emit fileReceiveCompleted(completedPath);  // 전체 경로 전달
                cleanupReceive();
                m_receiveState = WaitingForHeader;
            }
        }
